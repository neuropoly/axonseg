...
gRatio_map=as_display_labelgRatio(axonlist,PixelSize,'gRatio',size(img));
axonEquivDiameter_map=as_display_labelgRatio(axonlist,PixelSize,'axonEquivDiameter',size(img));
RGB = ind2rgb8(gRatio_map,hot(100));
imwrite(0.5*RGB+0.5*repmat(img,[1 1 3]),[output 'gRatio_0_1.tif'])
RGB = ind2rgb8(axonEquivDiameter_map,hot(80));
imwrite(0.5*RGB+0.5*repmat(img,[1 1 3]),[output 'axonEquivDiameter_0_8.tif'])
